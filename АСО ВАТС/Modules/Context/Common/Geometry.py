...
class Rect:
    ...
    @staticmethod
    def split_rect(vertices, split=4):
        (x1, y1), (x2, y2), (x3, y3), (x4, y4) = vertices

        width = x2 - x1
        height = y4 - y1

        small_width = width // split
        small_height = height // split

        result = []
        for i in range(split):
            for j in range(split):
                left_x = x1 + j * small_width
                bottom_y = y1 + i * small_height
                small_square = Rect(left_x, bottom_y, small_width, small_height)
                result.append(small_square)
    ...
    def get_scaled_vertices(self, vertices=None) -> list[tuple[float, float]]:
        if vertices is None:
            vertices = self.get_vertices()
        return [(round(x * c.FIELD_TO_WINDOW_SCALE), round(y * c.FIELD_TO_WINDOW_SCALE)) for x, y in vertices]

    def get_translated_vertices(self, vertices=None) -> list[tuple[float, float]]:
        if vertices is None:
            vertices = self.get_vertices()
        return [(x + c.FIELD_OFFSET_X, y + c.FIELD_OFFSET_Y) for x, y in self.get_scaled_vertices(vertices)]
