...
from .vendor import UDPDigitalDriver
...
class AGTSHookAp:
    ...
    def do_move(self, current_waypoint, current_position: tuple = None, autorotate=True):
        dist = 10
        original_waypoint = current_waypoint
        while dist > self.proximity_threshold:
            ...
            if self.context.mission.cyb_status["CybP_02"] == "active":
                current_waypoint = {"x": 2.6, "y": 1.8}
            else:
                current_waypoint = original_waypoint

            dist = self.get_distance_to_target(current_position_x, current_position_y, current_waypoint)
            d1 = current_waypoint["x"] - current_position_x
            d2 = current_waypoint["y"] - current_position_y
            ...
            if k > 1:
                ...
                d1 = d1 * inv_k
                d2 = d2 * inv_k
            slip = self.context.mission.mission_vars.get("slip_direction", 0)
            target_direction_diff = (
            ...
            control = self.rotation_pid(target_direction_diff) if (autorotate or slip != 0) else 0
