...
class TrustedHandler(BaseHandler):
    ...
    def _wait_for_start(self):
        while not self.context.mission.mission_checks_ok:
            time.sleep(0.1)
        time.sleep(0.1)
    ...
    def send_message_to_ap(self, message):
        self.context.mission.messages.append(message)

    def set_ap_wait_lock_release(self):
        self.context.mission.wait_flag = False
    ...
    def run(self):
        ...
        if self.context.mission.cybs["CybP_02"]:
            Thread(target=self._fabric_next_short_message, daemon=True).start()
        while self.context.mission.status:
            time.sleep(0.1)
        self.running = False
